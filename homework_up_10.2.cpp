...
public:
	big_number(const char* s = "") : ptr(nullptr) //const char* s = "" - это значит мы содали указатель s на неизменяемые данные, и эти данные пустота. 
		...
		// По умолчанию объект будет инициализирован пустой строкой. 
		// массив ptr имеет размер 1, там лежит только нуль-терминатор. 
	{
		if (s)
		{
	...
	
	{
		...
		for (int i = 0; i < size; ++i)             //складываю два числа типа int, которые лежат в массивах типа char, последние в складываемых объектах
		{
			...
			p *= 10;
		}                                         // сумма этих двух чисел находится в переменной n          
		
		char* p1 = new char[size+ 1];             // создал вспомогательный массив нужной длины

		big_number temp(p1); // создал временный объект, нужно, чтобы это был объект длиной равной size
		...
		delete[] p1;                // освобождаю память от вспомогательного массива char p1
	...
	int mult = other;
		...
		//std::cout << "Операция умножения на число" << std::endl;  // операция умножения на число проходит нормально, теперь нужно вывести на печать
		                                                          // объект типа big_number, для этого нужно полученный результат положить в массив типа чар 
		...
		char* p2 = new char[num + 1];    // создал вспомогательный массив нужной длины
		big_number temp(p2);             // создал временный объект 
		...
		delete[] p2;                 // Освобождаю память от вспомогательного массива char p2
		return temp;                // возвращаю объект temp, как результат сложения двух чисел и преобразования суммы в массив типа char
	}
...
int main()
{
	...
	big_number result;
	result = number11 + number12; // Работает хорошо.
	...
	int mult;
	mult = 1566;
	result1 =  number14 * mult; // Тестирование показало, что всё работает хорошо. 
